...
- **`src/content.ts`** — adds `import * as probe from './debug/probe';` and inserts **15 total probe calls** (11 `mark` + 2 `reset` + 2 `dump`): in `runPreload`, a `probe.reset()` + `PRELOAD_START` mark outside a try/finally wrapping the storage read, `PRELOAD_DONE` mark inside the success branch, and `probe.dump('preload')` in the finally so the preload envelope ALWAYS fires (including after a failed read) instead of having its marks silently wiped by the next `updateLinks` scan's `probe.reset()`. Inside `updateLinks`: a `probe.reset()` + `SCAN_START` mark outside the try block (so SCAN_START is always the first entry even on thrown scans), `PENDING_COLLECTED` after the cached/uncached split, `FAST_PATH_PAINTED` on both the full-hit early-return branch and the mixed-case split, `PORT_SEND` before the portFetcher call, `PORT_FIRST_CHUNK` inside the `distributeChunk` closure on first invocation (gated by a closure-scoped `firstChunkSeen` flag), `PORT_DONE` after the portFetcher resolves, and `PAINT_DONE` before the final `probe.dump('scan')`. The entire body of `updateLinks` post-reset is wrapped in `try { ... } finally { probe.dump('scan'); }` so every scan — success, exception, or early return — produces exactly one envelope.
...
- **15 total probe calls in content.ts** (11 `mark` + 2 `reset` + 2 `dump` — the pair of each in `runPreload` and in `updateLinks`, each with its own try/finally-guarded dump). Coarse phases beat fine profiling for this release's goal. Adding mark-per-appendChild or mark-per-queryParse would bloat the output and make DCE verification harder to read.
