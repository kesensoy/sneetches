...
- **13 total probe calls in content.ts** (11 `frame.mark` + 2 `frame.dump`, plus 2 `probe.newFrame` — one pair in `runPreload` and one in `updateLinks`, each with its own try/finally-guarded dump). Coarse phases beat fine profiling for this release's goal. Adding mark-per-appendChild or mark-per-queryParse would bloat the output and make DCE verification harder to read.
