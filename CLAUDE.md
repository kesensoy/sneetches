...
- **`scripts/probe-run.ts`** — new file (~290 lines). Node CLI that: (1) parses `--url` / `--label` args, (2) copies the user's real Chrome profile to a temp dir (`SNEETCHES_CHROME_PROFILE_SRC` env override supported, cross-platform default paths for darwin/linux/win32), (3) runs `npm run dev` if `build/` is missing or >60s old, (4) sanity-checks that `build/content.js` actually contains `SNEETCHES_PROBE`, (5) spawns Chrome via `child_process.spawn` with `--remote-debugging-port=9222 --user-data-dir=<temp> --load-extension=<build>`, (6) waits for the debug port via polled `http://localhost:9222/json/version` with a 10s deadline, (7) attaches via `chrome-remote-interface`, (8) subscribes to `Runtime.consoleAPICalled` filtered on `args[0].value === 'SNEETCHES_PROBE'`, (9) parses captured payloads, (10) waits 500ms post-first-capture for drain, (11) writes to `docs/plans/probe-runs/<timestamp>-<slug>.json` (directory gitignored as part of `docs/plans/`), (12) prints an ASCII diff table per context against the most recent previous run, (13) kills Chrome cleanly in `finally` + `SIGINT`/`SIGTERM`/`uncaughtException` handlers and `rm -rf`s the temp profile.
...
- **`eslint.config.mjs`** — adds a scoped override block for `scripts/**/*.ts` that disables `@typescript-eslint/no-require-imports` (the `chrome-remote-interface` package is CJS-first and there's no clean ESM path) and `@typescript-eslint/no-unused-vars` (belt-and-suspenders for future skeleton-first-wire-later tasks). `src/` and `tests/` trees are unaffected.
- **`package.json`** — adds devDependencies `terser-webpack-plugin@^5.4.0`, `chrome-remote-interface@^0.33.3`, `@types/chrome-remote-interface@^0.33.0`, `ts-node@^10.9.2`. Adds scripts `probe` (runs the measurement script via ts-node) and `test:dce` (builds prod and greps for probe leak).
...
### Key design decisions (locked in during 2026-04-15 brainstorm)
- **Raw CDP via `chrome-remote-interface`, not Playwright, not chrome-devtools MCP.** chrome-devtools MCP is stdio-interactive-only and has a known habit of dumping Authorization headers into agent-visible output (already burned a PAT on 2026-04-14). Playwright's `connectOverCDP` fights the "attach to user's real Chrome" use case — its abstraction is oriented around launching its own Chromium, not attaching to whatever's already running. Raw CDP is the smallest interface that does what we need.
- **Chrome profile is COPIED to a temp dir at probe time, not used live.** Chrome locks its profile dir; corrupting the real profile is an unacceptable failure mode. Cross-platform default path detection with `SNEETCHES_CHROME_PROFILE_SRC` env override for non-default layouts.
...
Once 1.1.6 ships and `npm run probe` is a one-command loop, the two deferred 1.1.4 perf candidates become cheap to evaluate: (a) **SW keepalive ping** — write it, probe before/after, read the diff, decide in an afternoon instead of a multi-day investigation; (b) **content-script Worker architecture** — the same before/after workflow measures the real savings (research estimated ~500ms–1s) before committing to 2–3 days of implementation. Neither is part of 1.1.6; both benefit from 1.1.6 existing.
