...
### What changed
- **`src/debug/probe.ts`** — new file. Exports `Phase` constants (12 phase names covering preload/scan/pending/fast-path/port/paint on the content-script side and handler-entry/fetch-start/fetch-done on the service worker side), a `ProbeFrame` class with `mark(phase, extra?)` and `dump()` methods plus a public-readonly `entries` array, and a `newFrame(label)` factory. Each scan holds its own `ProbeFrame` instance in a local variable so concurrent scans (which interleave on `await` points inside `updateLinks`) each mark into their own frame without cross-scan interference. `mark` pushes `{phase, t: performance.now(), extra}`; `dump` emits exactly one `console.log('SNEETCHES_PROBE', JSON.stringify(payload))` and clears the frame's entries. Every method guards on `__DEBUG__` so production builds are fully stripped. Debug builds also mount `globalThis.sneetchesProbe = { newFrame, Phase }` for manual invocation from DevTools on any page without reloading.
...
- **`webpack.config.js`** — rewritten from plain-object form to function form `(_env, argv) => config` so it can read `argv.mode`. Adds `DefinePlugin` substituting `__DEBUG__` for `true` (development) or `false` (production). Adds `TerserPlugin` in `optimization.minimizer` with `terserOptions.compress.pure_funcs: ['probe.newFrame', 'frame.mark', 'frame.dump']` and `passes: 2` so call sites are deleted entirely from the prod bundle — not just their bodies. `devtool` is now keyed off `isProd` rather than the old `process.env.NODE_ENV` check. The `pure_funcs` names are keyed on `import * as probe from ...` AND on a local variable named `frame` at call sites — if either shape changes, `npm run test:dce` catches it. Verified via `npm run test:dce` that no `SNEETCHES_PROBE` string literal survives in `build/*.js`.
- **`jest.config.js`** — adds `globals: { __DEBUG__: true }` so unit tests see the live code path at the same value as development builds, and registers `./tests/probe-silence.ts` in `setupFiles` to swallow `SNEETCHES_PROBE` console output in non-probe tests (the probe module fires dumps during any test that exercises `content.ts` or `service-worker.ts`, which would otherwise drown test output in 58+ envelope logs per run).
- **`tests/probe-silence.ts`** — new file. One-time Jest setup hook that wraps `console.log` to drop any call whose first argument is `'SNEETCHES_PROBE'`. Probe-specific tests (`tests/probe.test.ts`) use `jest.spyOn(console, 'log')` which replaces the function wholesale, bypassing this wrapper, so probe envelope assertions still work.
- **`src/content.ts`** — adds `import * as probe from './debug/probe';` and instruments both `runPreload` and `updateLinks` with their own per-invocation `ProbeFrame` instances. `runPreload`: `const frame = probe.newFrame('preload')` outside a try/finally wrapping the storage read, `frame.mark(PRELOAD_START)` then `frame.mark(PRELOAD_DONE)` on success, and `frame.dump()` in the finally so the preload envelope ALWAYS fires (including after a failed read). `updateLinks`: `const frame = probe.newFrame('scan')` outside the try block; inside the try, `SCAN_START` (first mark, always emitted), `PENDING_COLLECTED` after the cached/uncached split, `FAST_PATH_PAINTED` on both the full-hit early-return branch and the mixed-case split, `PORT_SEND` before the portFetcher call, `PORT_FIRST_CHUNK` inside the `distributeChunk` closure on first invocation (gated by a closure-scoped `firstChunkSeen` flag), `PORT_DONE` after the portFetcher resolves, and `PAINT_DONE` before the final `frame.dump()`. The `distributeChunk` closure captures `frame` by reference so its marks land on the right scan's frame. The try/finally wrapping guarantees every scan produces exactly one envelope regardless of success, exception, or early return.
- **`src/service-worker.ts`** — adds the probe import and restructures `handleFetchReposRequest` with a per-request `ProbeFrame`: `const frame = probe.newFrame('sw')` outside the try block, then inside: `frame.mark(SW_HANDLER_ENTRY, { nwos: req.nwos?.length ?? 0 })` (the optional chaining + nullish-coalesce hardens against malformed port messages where `req.nwos` might be undefined), `SW_FETCH_START` after `getAccessToken()`, `SW_FETCH_DONE` after `fetchRepoDataStreaming`, and `frame.dump()` in the `finally` block. Concurrent port connections each call `handleFetchReposRequest` and each get their own local `frame`, so SW-side concurrency is structurally isolated the same way CS-side is.
- **`scripts/probe-run.ts`** — new file (~400 lines). Node CLI that: (1) loads `.env.probe` at the repo root for `SNEETCHES_PROBE_GITHUB_PAT`, (2) parses `--url` / `--label` args (both require explicit values or throw), (3) rebuilds in dev mode if `build/content.js` is missing, older than the newest file under `src/` (mtime-based so post-edit probe runs always see fresh code), or a production bundle (no `SNEETCHES_PROBE` envelope in the output), (4) launches **Chrome for Testing** via **Puppeteer 24.41+** with `enableExtensions: [buildPath]` + `pipe: true` (required for the path-list form) and a persistent `userDataDir` at `<repo>/.sneetches-probe/profile/` (gitignored, co-located with the code that uses it, easy to `rm -rf`), (5) locates the extension's service-worker target via `browser.waitForTarget(t => t.type() === 'service_worker')`, (6) injects the PAT into `chrome.storage.sync` via `worker.evaluate(async (token) => chrome.storage.sync.set({ access_token: token, token_validated: true }))` — only if not already set, so repeat runs don't wipe the extension's persistent cache through `handleSyncStorageChange`, (7) **wires two separate console capture routes**: a `page.on('console', ...)` listener on the content-script target AND a dedicated `target.createCDPSession()` + `Runtime.consoleAPICalled` listener on the service-worker target (necessary because `page.on('console')` does NOT receive SW console events — they live in a separate target protocol stream), both feeding a shared `ingestEnvelope` function, (8) navigates to the target URL with `waitUntil: 'domcontentloaded'`, (9) waits 3 seconds for React hydration to finish on awesome-list pages (measured 2026-04-15), (10) gates capture on "real" payloads — a payload counts as real if it's a SW envelope or contains a `pending-collected` mark (i.e., found at least one repo anchor and went past the fast-path split), (11) drains 500ms past the last real capture, (12) writes ALL captured payloads (real + empty pre-hydration scans + preload envelope + SW envelope) to `docs/plans/probe-runs/<timestamp>-<slug>.json`, (13) prints an ASCII diff table per context against the most recent previous run, (14) closes the browser in `finally`. Chrome window is positioned off-screen at `--window-position=-10000,-10000` so agent-driven runs don't visually disrupt the desktop.
...
- **`tests/probe.test.ts`** — new file. 19 unit tests covering Phase constants, frame construction, `mark` shape, `dump` envelope format, `pageUrl` sanitization, clear-after-dump behavior, production-mode (`__DEBUG__ === false`) no-op contract, and — critically — two real-async-interleave tests that run two or three concurrent `Promise.all`'d "scan" functions through actual `await`s between marks. The concurrent tests pin the correctness of the per-scan-frame design against the reviewer-flagged case where a shared-state stack breaks under mid-await resume.
- **Test suite grew from 215 to 234** (19 new probe-module tests).
...
- **15 total probe calls in content.ts** (11 `frame.mark` + 2 `frame.dump` + 2 `probe.newFrame` — one set in `runPreload` and one in `updateLinks`, each with its own try/finally-guarded dump). Coarse phases beat fine profiling for this release's goal. Adding mark-per-appendChild or mark-per-queryParse would bloat the output and make DCE verification harder to read.
